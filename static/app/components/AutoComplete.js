Ext.define('Kitchensink.components.AutoComplete', {
	...
	initialize: function() {
		...
		var doSearchWithTimeout = function() {
			...
			searchTimeout = setTimeout(function() {
				var uriString = that.config.config.proxy.url + (that.config.config.proxy.url.indexOf('?')!=-1 ? '&' : '?') + encodeURIComponent(that.config.config.needleKey) + '=' + encodeURIComponent(that.getValue(true));
